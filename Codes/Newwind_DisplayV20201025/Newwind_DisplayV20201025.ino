...
void callback(char* topic, byte* payload, unsigned int length) {
 ...
 /*
  for (JsonPair keyValue : documentRoot) {
    ...
    //Serial.println(keyValue.value().c_str());
  ...
  */
  ...
  {
    JSONmessageBuffer[i] =char(payload[i]) ; 
    // Serial.print(char(payload[i]));
  }
 // Serial.print("] \n");
  JSONmessageBuffer[length] = 0x00 ;
  */
  /*
  ...
  */
  /*
  DeserializationError error = deserializeJson(doc, JSONmessageBuffer);
  ...
  */
  ...
  Serial.println(a6);
  //--------------------
 Windspeed = ChartoString(a3).toDouble();
 Winddir = ChartoString(a4).toInt();
 Temp = ChartoString(a5).toDouble();
 Humid = ChartoString(a6).toDouble();
 //     SendImage(sitetitle) ;  
 // ClearScreen() ;       
  //    delay(500);
