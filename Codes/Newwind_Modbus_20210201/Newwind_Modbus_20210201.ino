...
void loop() {
 // mqttclient.loop() ;
  ...
  payloadStr = CreateJsonData2(MacData,IpAddress2String(ip),Windspeed,Windangle,Temp,Humid) ;
   ...
   connectstr = "" ;
   //http://ncnu.arduino.org.tw:9999/wind/windadd.php?mac=246F289E432C&ip=192.168.1.102&speed=0.00&way=0&temp=0.00&humid=0.00
 ...
 connectstr = "?mac=" + MacData+"&ip="+IpAddress2String(ip)+"&speed="+ String(Windspeed)+"&way="+ Windangle+"&temp="+ String(Temp)+"&humid="+ String(Humid);
  ...
  Serial.print("Size is :") ;
   ...
   Serial.print("/") ;
      ...
      Serial.print("\n") ;
if (mqttclient.publish(&pubTopic[0],JSONmessageBuffer,payloadStr.length()+1))
{
      ...
      while(true)
        {
            ...
            if (Serial2.available() >= 7)
                {
                    Serial2.readBytes(incomingdata1, 7) ;
                      ret = true ;
                      return  ret ;
                }
        }
      ...
      while(true)
        {
            ...
            if (Serial2.available() >= 9)
                {
                    Serial2.readBytes(incomingdata2, 9) ;
                      ret = true ;
                      return  ret ;
                }
        }
      ...
      while(true)
        {
            ...
            if (Serial2.available() >= 9)
                {
                    Serial2.readBytes(incomingdata3, 9) ;
                      ret = true ;
                      return  ret ;
                }
        }
